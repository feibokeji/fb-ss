...
<!-- /bpr/brand/modify.jsp -->
<body>
	<form id="modifyForm" name="modifyForm" method="post" action="${contextPath }/bpr/brand/modifySave">
		<input type="hidden" name="uid" value="${brand.uid }"/>
		<input type="hidden" name="clogo" value="${brand.clogo }"/>
		<input type="hidden" name="isort" value="${brand.isort }"/>
		<table class="add_update_table">
			<tr>
				<th><span style="color: red;">*</span>品牌名称：</th>
				<td><input type="text" id="modifyCname" name="cname" class="l-text validate[required,funcCall[ajaxCname]]" value="${brand.cname }"/></td>
			</tr>
			<tr>
				<th>官方网站：</th>
				<td colspan="3"><input type="text" id="modifyCurl" name="curl" class="l-text validate[custom[url]]" value="${brand.curl }" style="width:470px;"/></td>
			</tr>
			<tr>
				<th>描述：</th>
				<td colspan="3"><textarea rows="2" cols="71" id="modifyCdesc" name="cdesc" class="l-textarea">${brand.cdesc }</textarea></td>
			</tr>
			<tr>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
  	...
  	<!-- javascript文件引用 -->
	<script type="text/javascript">var contextPath = '${contextPath}';var oldCname = '${brand.cname}';</script>
	...
	<script type="text/javascript" src="${contextPath}/common/js/bpr/brand/modify.js"></script>
</body>
</html>