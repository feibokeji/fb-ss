...
<body>
	<div>
		...
		<table class="listTable">
			<tr>
				...
				<td align="left" style="border-bottom: 1px solid #000000;">${order.csuppliername }</td>
		...
		<c:if test="${order.itype != 2 }">
		<!-- 应收应付信息 -->
		<div style="margin: 0 auto;width: 99%;margin-top: 5px;">
			<h2>
				<c:if test="${order.itype eq 0 }">应付信息</c:if>
	    		<c:if test="${order.itype eq 1 }">应收信息</c:if>
			</h2>
		</div>
		<table class="listTable" style="margin-top: 5px;">
			<tr>
				<th width="30px">序号</th>
				<th width="80px">供应商信息</th>
			...
			<tr>
				<td>${i.index+1 }</td>
				<td align="left">${receivable.csuppliername }</td>
		...
		<div style="margin: 0 auto;width: 99%;margin-top: 5px;">
			<h2>
				<c:if test="${order.itype eq 0 }">实付信息</c:if>
	    		<c:if test="${order.itype eq 1 }">实收信息</c:if>
			</h2>
		</div>
		<table class="listTable" style="margin-top: 5px;">
			<tr>
				...
				<c:if test="${order.itype != 0 }"><th width="80px">付款方式</th></c:if>
				<th width="80px">操作员</th>
				<th width="80px">类型</th>
		...
		<div style="margin-top: 20px;margin-bottom: 20px;text-align: center;">
			...
			<c:if test="${order.istatus eq 0 }">
				<input type="button" class="l-button" value="反审核" onclick="reverseAuditSupplierOrder()"/>
			</c:if>
			<!-- 未审核情况下功能按钮 -->
			<c:if test="${order.istatus eq 1 }">
				<input type="button" class="l-button" value="修改" onclick="modifySupplierOrder()"/>
				<input type="button" class="l-button" value="审核" onclick="auditSupplierOrder()"/>
				<input type="button" class="l-button" value="删除" onclick="deleteSupplierOrder()"/>
			</c:if>
	...
	<script type="text/javascript" src="${contextPath}/common/js/bpr/supplierOrder/view.js"></script>
</body>
</html>