...
$(function(){
	...
	brandTable = window['maingrid'] = $("#maingrid").ligerGrid({
        columns: [
            ...
            { display: '品牌名称', name: 'cname', width: 120, minWidth: 100, align:'left' },
            { display: '官方网站', name: 'curl', align:'left', width: 300,minWidth: 280, align:'left' },
            { display: '描述', name: 'cdesc', align:'left', width: 300,minWidth: 280, align:'left' }
            ],
...
function addBrand()
{
	brandDialog = $.ligerDialog.open({url:contextPath+"/bpr/brand/add",title:"新增品牌",allowClose:true,width:640,height:320});
}
function modifyBrand()
{
	var rows = brandTable.getCheckedRows();
	if(rows != null && rows != ""){
		brandDialog = $.ligerDialog.open({url:contextPath+"/bpr/brand/modify?uid="+rows[0].uid,title:"修改品牌",allowClose:true,width:640,height:320});
	}
	else
	{
