...
function add(input_element,i){
	//销售量
	var inventoryQuantity = parseFloat($("#orderDetailList_"+i+"_maxQuantity").val());
	//价格
...
function submitForm(){
	if($("#checkStandForm").validationEngine("validate")){
		...
		if(quantity > 0){
			$.ligerDialog.confirm("您确定要退货吗？",function(yes){
				if(yes){
					var waitting = $.ligerDialog.waitting('退货中,请稍候...');
					$("#checkStandForm").ajaxSubmit(function(data){
						waitting.close();
						if(data == "fail")
							$.ligerDialog.error("退货失败!");
						else{
							window.parent.returnGoodsDialog.close();
						}
					});
				}
			});
		}else{
			$.ligerDialog.error("退货量不能为零!");
		}
	}
}