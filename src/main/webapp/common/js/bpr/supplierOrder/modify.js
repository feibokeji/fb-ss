...
$(function(){
	...
	switch (itype) {
	case 0:
		$("#titleH2").text("商品入库单修改");
		break;
	case 1:
		$("#titleH2").text("商品退库单修改");
		break;
	case 2:
		$("#titleH2").text("商品报损单修改");
		break;
	default:
		$("#titleH2").text("商品入库单修改");
		break;
	}
	$("#modifyOrderForm").validationEngine('attach', { 
		 promptPosition: 'bottomRight',scroll:false
	});
	var csuppliernameComboBox = $("#modifyCsuppliername").ligerComboBox({
		...
		{
			$("#modifyUsupplierid").val(value);
		}
	}).setText(csuppliername);
	$("#modifyDrecordateStr").ligerDateEditor().setValue(drecorddateStr.substring(0,10));
	lineNumber = parseInt($("#listSize").val());
	for(var i = 1; i <= lineNumber; i++){
		var _element = "modifyOrderDetail_" + i + "_cname";
...
function createLine(i){
	var _line = "<tr id='dataLineTr"+i+"'>" +
					...
					"<td><input type='text' style='width: 80px;height:18px;text-align:center;border:none;' id='modifyOrderDetail_"+i+"_ccategoryname' name='orderDetailList["+i+"].ccategoryname'/></td>" + //类别
					"<td><input type='text' style='width: 80px;height:18px;text-align:center;border:none;' id='modifyOrderDetail_"+i+"_cbrandname' name='orderDetailList["+i+"].cbrandname'/></td>" + //品牌
					"<td><input type='hidden' id='modifyOrderDetail_"+i+"_uothergoodsid' name='orderDetailList["+i+"].uothergoodsid'/><input type='text' id='modifyOrderDetail_"+i+"_cname' name='orderDetailList["+i+"].cname' style='width: 100px;'/></td>" +//:名称
					"<td><input type='text' style='width: 60px;height:18px;text-align:center;border:none;' id='modifyOrderDetail_"+i+"_cunitname' name='orderDetailList["+i+"].cunitname'/></td>" + //单位
					"<td><input type='text' style='width: 60px;height:18px;text-align:right;border:none;' id='modifyOrderDetail_"+i+"_nquantity' name='orderDetailList["+i+"].nquantity' onchange='updateLineAmount("+i+")' class='validate[custom[number],min[0]]'/></td>" +//:数量
					"<td><input type='text' style='width: 60px;height:18px;text-align:right;border:none;' id='modifyOrderDetail_"+i+"_nprice' name='orderDetailList["+i+"].nprice' onchange='updateLineAmount("+i+")' class='validate[custom[number],min[0]]'/></td>" +//:价格
					"<td><input type='text' style='width: 60px;height:18px;text-align:right;border:none;' id='modifyOrderDetail_"+i+"_namount' name='orderDetailList["+i+"].namount' onchange='updateLinePrice("+i+")' class='validate[custom[number],min[0]]'/></td>" +//:金额
...
function loadAutoComplete(elementId,_number){
	var _val = $("#"+elementId).val();
	var _comboBox = $("#" + elementId).ligerComboBox({
		slide: false,
		width:200,
		height:18,
        selectBoxWidth: 600, 
        	...
        	if(box_data != null){
        		for(var i = 0; i < box_data["Rows"].length;i++){
        			if(box_data["Rows"][i].uid == value){
        				$("#modifyOrderDetail_" + _number + "_ccategoryname").val(box_data["Rows"][i].ccategoryname);//:类别
        				$("#modifyOrderDetail_" + _number + "_cbrandname").val(box_data["Rows"][i].cbrandname);//:品牌
        				$("#modifyOrderDetail_" + _number + "_uothergoodsid").val(value);//:商品主键
        				$("#modifyOrderDetail_" + _number + "_cname").val(box_data["Rows"][i].cname);//:商品名称
        				$("#modifyOrderDetail_" + _number + "_cunitname").val(box_data["Rows"][i].cunitname);//:单位
        				$("#modifyOrderDetail_" + _number + "_nprice").val(box_data["Rows"][i].ncostprice);//:商品进货价
        				$("#modifyOrderDetail_" + _number + "_nquantity").val(1);//:商品数量
        				$("#modifyOrderDetail_" + _number + "_namount").val(box_data["Rows"][i].ncostprice);//:商品金额
...
function updateLineAmount(number){
	var nprice = $("#modifyOrderDetail_" + number + "_nprice");//:价格
	var nquantity = $("#modifyOrderDetail_" + number + "_nquantity");//:数量
	var namount = $("#modifyOrderDetail_" + number + "_namount");//:金额
...
function calculateTotalAmount(){
	...
	for(var i = 1; i <= findMaxIndex(); i++){
		var _amount = $("#modifyOrderDetail_" + i + "_namount").val();
...
function addLine(elementId){
	...
	//数据行添加时间
	var _element = "modifyOrderDetail_" + totalNumber + "_cname";
...
function clearLine(){
	for(var i = 1;i <= findMaxIndex();i++){
		$("#modifyOrderDetail_" + i + "_ccategoryname").val("");
		$("#modifyOrderDetail_" + i + "_cbrandname").val("");
		$("#modifyOrderDetail_" + i + "_uothergoodsid").val("");
		$("#modifyOrderDetail_" + i + "_cname").val("");
		$("#modifyOrderDetail_" + i + "_cunitname").val("");
		$("#modifyOrderDetail_" + i + "_nprice").val("");
		$("#modifyOrderDetail_" + i + "_nquantity").val("");
		$("#modifyOrderDetail_" + i + "_namount").val("");
		calculateTotalAmount();
	}
}
/**
...
function validData(){
	for(var i = 1; i <= findMaxIndex(); i++){
		var _element = "modifyOrderDetail_" + i + "_uothergoodsid";
 ...
 */
function submitForm(){
	if($("#modifyOrderForm").validationEngine("validate")){
		if(validData()){
			$.ligerDialog.confirm("确定要保存吗？",function(yes){
				if(yes){
					var waitting = $.ligerDialog.waitting('数据保存中,请稍候...');
					$("#modifyOrderForm").ajaxSubmit(function(data){
						waitting.close();
						if(data == "fail")
...
function loadChooseOtherGoods(selectedRows){
	...
	if(validTotalNumber < (rowsLength + validLine)){
		for(var i = 0; i < (rowsLength + validLine) - validTotalNumber; i++){
			...
			var _element = "modifyOrderDetail_" + maxIndex + "_cname";
