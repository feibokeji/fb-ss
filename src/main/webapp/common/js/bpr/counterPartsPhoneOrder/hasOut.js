...
$(function(){
	...
	//已调出手机信息列表信息
	ligerGrid = window['hasOutMainGrid'] = $("#hasOutMainGrid").ligerGrid({
		columns: [
		...
		url:contextPath + "/bpr/counterPartsPhoneOrder/getCounterPartsPhoneOrderJSON?itype=4",
...
function inOrder(){
	...
	if(rows != null && rows != ""){
		$.ligerDialog.confirm("确定要从同行调回此手机吗？",function(yes){
			if(yes){
				...
				$.ajax({
					...
					async:false,
					cache:false,
					url:contextPath+"/bpr/counterPartsPhoneOrder/executeInOrder",
					data:{'uid':rows[0].uid},
					dataType:"html",
